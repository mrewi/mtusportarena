  ...
  useEffect(() => {
    const fetchFixtures = async () => {
      try {
        ...
        fixturesList.sort((a, b) => (a.date?.seconds || 0) - (b.date?.seconds || 0));
        // Group fixtures by game week
        const groupedFixtures = fixturesList.reduce((acc, fixture) => {
        ...
        setFixturesByWeek(groupedFixtures);
      } catch (error) {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <>
          {Object.entries(fixturesByWeek).map(([gameWeek, fixtures]) => (
            <Box key={gameWeek} sx={{ textAlign: 'center', marginBottom: 4, padding: 2 }}>
              <Typography variant="h4" gutterBottom>{`Game Week ${gameWeek}`}</Typography>
              <Grid
                container
                spacing={3}
